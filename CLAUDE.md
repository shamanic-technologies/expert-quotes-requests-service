...
- **API host is `www.connectively.us`, NOT `featured.com`.** Featured.com relaunched as Connectively (2026-06-02) and put the old `featured.com` host behind a Vercel firewall: every server-to-server call now returns `429` with a `Vercel Security Checkpoint` JS-challenge HTML page (not JSON), which `featured-client.ts` correctly propagates as `502` (no silent fallback). The `external-users` API itself migrated verbatim to `https://www.connectively.us/api/external-users` — **same paths, same `x-access-token` login (24h), same payloads** (proven live with platform creds 2026-06-03: `/login`→200+JWT, `/premium-question-list`→200 n=705, `/opportunities-list`→200 n=916; old host →429 on all). `DEFAULT_BASE_URL` + `config.featuredApiBaseUrl` point at the Connectively host; `FEATURED_API_BASE_URL` is set on Railway prod as the explicit override. The official Connectively API positions itself **read-only** (docs FAQ: "the API does not support answer submissions") and deadlines are quoted in AZ/MST; our legacy `/answer-question` submit still works on the new host (same backend). Endpoint reference is gated behind "Request API Access" — not public.
...
**`x-campaign-id` (+ `x-brand-id` / `x-feature-slug` / `x-workflow-slug`) is the per-campaign cost-attribution key** that gates platform daily-budget pacing — campaign-service sums `runs_costs` via `runs.campaign_id`, so the `featured-api-pitch-submit` cost (and any chat-service LLM cost EQRS triggers) must land on a child run whose `campaign_id` is set, else $-spend falls into the NULL-campaign bucket and the gate runs uncapped (10-day leak: 220 actual pitch-submit rows / $440 on `campaign_id=NULL`). Cost ROWS already carried `x-campaign-id` via `costIdentityHeaders`, but `createChildRun` previously forwarded ONLY `x-audience-id` → the RUN row stayed `campaign_id=NULL`. Fixed v0.1.15: `createChildRun` forwards `x-campaign-id`/`x-brand-id`/`x-feature-slug`/`x-workflow-slug` (header takes precedence, else inherited from parent), `withRunTracking` threads `req.campaignId`/`brandId`/`featureSlug`/`workflowSlug` in. Synchronous path (cost declared in the answers.ts handler, no worker), so no job-row carrier needed. Same allowlist plumbing as `x-audience-id`; absent → omitted, never throws.
...
| `FEATURED_API_BASE_URL` | no | `https://www.connectively.us/api/external-users` | Override for staging / test. Default is the live Connectively host; the old `featured.com` host is firewalled (429 Vercel checkpoint) since the 2026-06-02 relaunch. |
